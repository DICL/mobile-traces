...
SELECT data, _id FROM _sync_state WHERE account_name='dicl.skku@gmail.com' AND account_type='com.google'
SELECT _id, minutes, method FROM Reminders WHERE (event_id='107') ORDER BY minutes ASC, method ASC
SELECT attendeeName, attendeeEmail, attendeeRelationship, attendeeStatus, attendeeIdentity, attendeeIdNamespace, attendeeType FROM Attendees, Events, Calendars WHERE (Events._id=Attendees.event_id AND Events.calendar_id=Calendars._id) AND (event_id='107') ORDER BY attendeeName ASC, attendeeEmail ASC
BEGIN EXCLUSIVE;
UPDATE Calendars SET dirty=1,visible=0,mutators='com.google.android.calendar' WHERE _id='3'
COMMIT;
...
BEGIN EXCLUSIVE;
COMMIT;
SELECT Instances.event_id AS event_id, title, eventLocation, begin, end, accessLevel, selfAttendeeStatus, account_type, _sync_id, original_sync_id, allDay FROM Instances INNER JOIN view_events AS Events ON (Instances.event_id=Events._id) WHERE (begin<='1642227207494' AND end>='1641881607494') AND ((visible=1) AND (calendar_access_level=700) AND (selfAttendeeStatus!=2) AND ((account_type!='com.google') OR (sync_data9 IS NULL) OR ((sync_data9!='true') AND ((CAST(sync_data9 AS INTEGER)&1)=0))))
