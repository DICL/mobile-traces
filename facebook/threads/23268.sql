...
BEGIN EXCLUSIVE;
UPDATE home_stories SET seen_state=1 WHERE (dedup_key='4434745127281735114' AND feed_type IN ('groups_tab'))
COMMIT;
BEGIN EXCLUSIVE;
UPDATE home_stories SET seen_state=1 WHERE (dedup_key='-2358428236100116262' AND feed_type IN ('groups_tab'))
COMMIT;
BEGIN EXCLUSIVE;
UPDATE home_stories SET seen_state=1 WHERE (dedup_key='4654149674887873403' AND feed_type IN ('groups_tab'))
COMMIT;
BEGIN EXCLUSIVE;
UPDATE home_stories SET seen_state=1 WHERE (dedup_key='-3857948435938683252' AND feed_type IN ('groups_tab'))
COMMIT;
BEGIN EXCLUSIVE;
